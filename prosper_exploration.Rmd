...
ggplot(aes(y=StatedMonthlyIncome, x=ProsperRating), 
...
I've removed the top 5% of the StatedMonthlyIncome data in order to better see the relationship between Income and Rating.  What I notice immediately are the horizontal lines that tells me income is quite constant throughout the rating range.  Even with overplotting, I can still distinguish the vertical lines.  So, same income can be seen in the bottom rating up to the top rating but there is overplotting in the middle of the income range compared to its lower and higher range.  In the 1st rating, although I still see the horizontal lines in the higher income range, there are less of them and as the rating goes up, more points can be seen in that side than in the lower side of the income range.  
...
I removed the top 5% and the relationship that pops out more is ProsperRating vs CreditScore where higher ratings have higher CreditScore.  The StatedMonthlyIncome is quite constant where the same horizontal lines can be seen in all the ProsperRating.  What I can conclude in this plot is that CreditScore influences the ProsperRating more than the StatedMonthlyIncome.
...
ggplot(aes(y=LoanAmtByTerm, x=ProsperRating), 
       data=prosper) +
  geom_jitter(aes(color=CreditScoreRange), alpha=0.20) +
  scale_color_brewer(type="div")  
...
#old plot, reviewer mentioned that line should only be use for time
#comparison, so changing to bar plot
#ggplot(aes(x=value, 
#           y=Quantile), 
...
ggplot(aes(y=value, 
           x=ProsperRating), 
       data=subset(prosper.by_rating.reshape, 
                   (!(variable %in% c("n","curBadRec")) & 
                      !(Quantile %in% c(0,1)) ) ) ) +
  geom_bar(aes(fill=factor(Quantile)), stat="identity") +
  facet_wrap(~variable, scales="free_y") +
  scale_y_sqrt(label=comma) +
  ggtitle("Summaries of the Positive Correlated Values by ProsperRating")


```

The diagram above shows the values for each Quantile at 0.25, 0.5 or median, and 0.75.
Comparing the quantile values for each ProsperRating, we can see that better ProsperRating have higher values than lower ProsperRating.
...
Furthermore, higher ProsperRating have higher medians in all the features, except for the features with negative correlation which are Inquiries, Delinquencies and Public Records.  
...
Majority of loans with low ProsperRating exhibits low value in Loan Amount, Bankcard Credit, Income and Principal Borrowed, at the same time they also have higher Inquiries, Delinquencies and Public Records, than loans with better ProsperRating.
...
### Description Two
This plot shows the quantile summaries per ProsperRating for each of the Positive-Correlated variables: Available Bankcard Credit, Monthly Income, Loan Amount, Loan Amount By Term, and Principal Borrowed.   

Generally, we can see that better ProsperRating have higher values at 0.25, 0.50 or median and 0.75 quantiles than the lower ProsperRating.  Furthermore, this increasing relationship is more pronounced in the Bankcard variable at all Quantile summary while in the other variables, the increasing value is more seen in the 0.75 quantile than in the 0.50 (median) and 0.25. This tells me that variance is greater between ProsperRating at 0.75 quantile mark.
...
ggplot(aes(y=CurrentBadRecords, 
  ...
  scale_color_brewer(type="div") +
  facet_wrap(~CreditScoreRange) +
  ggtitle("Bad Records vs LoanByTerm per ProsperRating by CreditScore")
